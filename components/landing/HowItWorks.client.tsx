...
type StageId = 'stage-1' | 'stage-2' | 'stage-3' | 'stage-4' | 'stage-5';

const STAGES: ReadonlyArray<{ id: StageId; num: number; label: string }> = [
...
>(function StageNav({ active, indicatorRef, onSelect }, ref) {
  return (
...
export function HowItWorks() {
  ...
  // Hash-based deep link (e.g. /#stage-3 lands on the Paper Analysis tab).
  // Only runs on mount; subsequent navigation is button-driven.
  useEffect(() => {
    const hash = typeof window !== 'undefined' ? window.location.hash.slice(1) : '';
    if (STAGES.some((s) => s.id === hash)) {
      setActiveStage(hash as StageId);
    }
  }, []);
  ...
  // Crossfade the panel content on tab switch. Re-runs on every
  // activeStage change because the dep is included in the useAnime
  // deps array.
      ...
      <Reveal>
        <div className="text-center py-16 px-6">
          ...
          <p className="text-slate-500 max-w-2xl mx-auto">
            ...
            no single node bottlenecks the network.
          </p>
        </div>
      </Reveal>
      <StageNav ref={navRef} active={activeStage} indicatorRef={indicatorRef} onSelect={setActiveStage} />

      {/* Active stage panel. `key` forces remount on switch so the
          entrance animation fires; the surrounding `panelRef` is what
          useAnime targets. */}
      <div
        ref={panelRef}
        key={activeStage}
        role="tabpanel"
        aria-label={STAGES.find((s) => s.id === activeStage)?.label}
        className="min-h-[420px]"
      >
        {activeStage === 'stage-1' && <Stage1 />}
        {activeStage === 'stage-2' && <Stage2 />}
        {activeStage === 'stage-3' && <Stage3 />}
        {activeStage === 'stage-4' && <Stage4 />}
        {activeStage === 'stage-5' && <Stage5 />}
      </div>

      {/* COMPOUNDING LOOP - sits below the tabbed pipeline; always visible. */}
